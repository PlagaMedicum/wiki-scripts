...
exists, then open that feature's `plan.md`. The current active plan is
`specs/001-docs-governance-hardening/plan.md` while `.specify/feature.json`
points there. If there is no active feature pointer, use the relevant
`specs/NNN-feature-name/` directory directly.
<!-- SPECKIT END -->
