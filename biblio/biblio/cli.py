...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="biblio",
        description="English operator CLI for reusable local bibliography replacers.",
        epilog=(
            "This tool was first developed for be.wiki, but the source folders and bot settings "
            "can be retargeted to another local wiki. Running with no subcommand launches the "
            "interactive startup wizard. The wizard also respects --no-color."
        ),
        parents=[common],
    )
    ...
    run_parser = subparsers.add_parser(
        "run",
        parents=[common],
        help="Run a bibliography replacer against wiki search results.",
    )
...
def _interactive_startup(ui: AppUI) -> int:
    return run_startup_wizard(ui, run_sources)
