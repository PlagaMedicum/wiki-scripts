...
from bewiki_biblio.manage import guess_candidate_defaults
...
def test_validate_reports_missing_canonical_files(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("bewiki_biblio.manage.project_root", lambda: tmp_path)
...
def test_add_source_creates_canonical_files(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("bewiki_biblio.manage.project_root", lambda: tmp_path)
    monkeypatch.setattr(
        "bewiki_biblio.ui.AppUI.prompt_text",
    ...
    monkeypatch.setattr("bewiki_biblio.ui.AppUI.prompt_csv", fake_prompt_csv)
    monkeypatch.setattr("bewiki_biblio.ui.AppUI.confirm", lambda self, label, default=True: True)
