...
pub(crate) fn build_child_command_owned(
    ...
    let log_filter = std::env::var("RUST_LOG").unwrap_or_else(|_| {
        "warn,bewiki_suppressor=info,hyper=warn,hyper_util=warn,h2=warn,reqwest=warn".to_string()
    });
...
mod tests {
    ...
    fn build_child_command_injects_supervisor_env() {
        ...
        let command =
            build_child_command(Path::new("/tmp/bewiki_suppressor"), &paths, &["run"]).unwrap();
